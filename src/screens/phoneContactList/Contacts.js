...
const ContactsList = props => {
  ...
  useEffect(() => {
    ...
    if (isUpdate || selectedContact) {
      props.navigation.navigate('PackagePickupAndDelivery', {
        data: { mobileNumber: selectedContact },
        update: 'mobile',
      });
    }
  ...
  const contactUpdate = item => {
    ...
    if (
      ...
      Snackbar.show({
        text: 'Invalid Contact! Please Select Valid Contact',
        duration: Snackbar.LENGTH_LONG,
      });
    } else {
      setSelectedContact(item?.phoneNumbers[0]?.number);
      setIsUpdate(true);
    }
  };
                ...
                renderItem={contact => {
                  ...
                  return (
                    <TouchableOpacity
                      onPress={
                        () => contactUpdate(item)
                        // handleContactList(item.phoneNumbers[0].number)
                      }>
